...
const Courses = () => {
    ...
    return (
        <div className={`lg:pt-10 pb-10 ${dark ? 'bg-slate-500' : ''}`}>
            <div className='container mx-auto  '>
                <div className="dropdown w-full mb-10">
                    ...
                    <ul tabIndex={0} className={`menu menu-compact dropdown-content bg-slate-50 mt-3 p-2 shadow  rounded-box w-max ${dark ? 'bg-slate-200' : ''}`}>
                        {
                ...
                <div className='grid lg:grid-cols-4 gap-10'>
                    ...
                    <div className={`border rounded-lg bg-slate-50 shadow-lg h-max hidden lg:block ${dark ? 'bg-slate-200' : ''}`}>
                        <h1 className={`bg-gray-200 rounded-t-lg px-3 py-2 text-xl font-semibold text-sky-600 ${dark ? 'bg-slate-300' : ''}`}>Course Categories</h1>
                        <div className='px-3 py-2 flex flex-col'>
                            {
                                categories.map(category => <button key={category.id} className={`mt-2 border-b py-1 hover:border-sky-600 hover:text-sky-600 flex items-center ${dark ? 'border-gray-400' : ''}`} onClick={() => handleShowenCourses(category.id)}><CodeBracketSquareIcon className='h-5 w-5 mr-1 text-gray-400' />{category.category}</button>)
                            }
                            <button onClick={() => handleShowenCourses('10')} className='mt-2 border-b py-1 hover:border-sky-600 hover:text-sky-600 flex items-center'><CodeBracketSquareIcon className='h-5 w-5 mr-1 text-gray-400' />All Courses</button>
                        </div>
                    </div>
